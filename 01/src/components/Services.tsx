...
export function Services() {
  return (
    <section id="servicos" className="relative py-24 md:py-36" aria-labelledby="services-title">
            ...
            return (
              <Reveal key={service.title} as="article" delay={index * 0.035}>
                <motion.div
                  ...
                  className={`premium-card group h-full rounded-3xl p-6 ${
                    index === 0 ? "animated-border blue-glow xl:col-span-2" : ""
                  }`}
                >
                  <div className="relative z-10 flex h-full min-h-64 flex-col justify-between">
                    <div>
                      <div className="mb-8 grid size-12 place-items-center rounded-2xl border border-[#3B82F6]/35 bg-[#14213D]/60 text-[#3B82F6] transition group-hover:scale-110">
                    ...
                    <div className="mt-8 h-px w-12 bg-[#3B82F6]/70 transition-all group-hover:w-24" />
                  </div>
                </motion.div>
              </Reveal>
            );
